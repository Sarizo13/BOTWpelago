...
- [ ] 🔧 **Save FRAÎCHE (Plateau) : les items s'ajoutent SANS déco/reco** — vraie cause (logs) : quand le jeu **réalloue le buffer pouch**, le client restait collé sur l'**ancien buffer** (qui paraît encore valide → staleness non détectée) → plus aucun nœud libre → « pool épuisé » jusqu'à une **déco/reco client** (qui forçait un re-scan trouvant le nouveau buffer). Fix : `refresh_inventory_if_stale` re-localise aussi quand le pool paraît épuisé, en **préférant un buffer AVEC nœuds libres** (`prefer_free`) → les créations repartent automatiquement, sans reconnect. → **à re-tester sur save neuve**
