...
- [ ] ℹ️ **« Le plateau isolé » reste actif = NORMAL** (pas un bug) : c'est la quête `FindDungeon`, archivée par `FindDungeon_Finish` — qui est **une location AP** (data/locations.json). On ne la force donc PAS (= déclencherait faussement le check). Elle se termine en clearant les 4 sanctuaires du plateau → le Roi → le check. **À vérifier** : le Roi se déclenche-t-il encore après un paravoile reçu d'AP ? (sinon check infaisable = décision de design).
