...
- [ ] 🔧 **Crash du JEU pendant une création live** — la liste de nœuds est scannée au début de `live_create_item` ; si le jeu **alloue le nœud libre** (ou déplace l'ancre) entre le scan et l'écriture, splicer corrompt la liste chaînée → crash. Fix : **re-validation juste avant d'écrire** (le nœud libre est-il TOUJOURS type 0xFFFFFFFF ? l'ancre pointe-t-elle vers un nœud plausible ?) → sinon reporté au poll suivant. À combiner avec le plafonnement du scan (moins de temps passé à écrire pendant que le jeu réalloue). → **à re-tester**
