...
- [x] ~~Garde de stabilité (`_INV_SETTLE_TIME`)~~ **RETIRÉE** — était basée sur une hypothèse fausse (la « transition » était en fait une vraie mort). Elle bloquait TOUT le filler sur save fraîche : `_relocate_inventory` remet `_inv_base=None` → `_locate_live_inventory` croit toujours que la base a changé → `_inv_changed_at` remis à jour chaque poll (pool épuisé → relocation) → la garde 4 s ne s'écoule jamais → filler jamais créé (silencieux). Les vrais crashes sont réglés ailleurs (orbe non créé, tri matériaux, contention fichier). La re-validation du nœud libre avant splice est conservée.
...
- [ ] 🔧 **Matériaux insérés à la BONNE place (tri sortKey)** — le pouch trie par `sortKey` (fruits 31xxx < viande 32xxx < matériaux 33xxx < poisson 34xxx < minerai 36xxx < monstre 37xxx). Avant : (1) insérer une sous-catégorie après une autre = désorganisation → crash ; (2) puis mon ancre « même préfixe » **reportait** le 1er item d'une sous-catégorie absente (items coincés en queue). Fix : ancre après le dernier nœud dont `(type, sortKey) ≤` celui du nouvel item = position triée exacte (table `data/item_sort_keys.json` via `tools/build_sort_keys.py`, 1734 entrées). → **à re-tester** : plus d'items coincés en queue, inventaire ordonné, pas de crash.
